...
/* 256x64 */
static const u8x8_display_info_t u8x8_sdl_256x64_info =
{
  /* chip_enable_level = */ 0,
  /* chip_disable_level = */ 1,
  /* post_chip_enable_wait_ns = */ 0,
...
uint8_t u8x8_d_sdl_256x64(u8x8_t *u8g2, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
  uint8_t x, y, c;
  uint8_t *ptr;
  switch(msg)
  {
    case U8X8_MSG_DISPLAY_SETUP_MEMORY:
      u8x8_d_helper_display_setup_memory(u8g2, &u8x8_sdl_256x64_info);
      u8g_sdl_init(256, 64);
      break;
    case U8X8_MSG_DISPLAY_INIT:
      u8x8_d_helper_display_init(u8g2);
...
void u8x8_Setup_SDL_256x64(u8x8_t *u8x8)
{
  u8x8_SetupDefaults(u8x8);
  u8x8->display_cb = u8x8_d_sdl_256x64;
  u8x8->gpio_and_delay_cb = u8x8_d_sdl_gpio;
  u8x8_SetupMemory(u8x8);
}
...
{
  static uint8_t buf[256*8];
  u8x8_Setup_SDL_256x64(u8g2_GetU8x8(u8g2));
  u8g2_SetupBuffer(u8g2, buf, 8, u8g2_ll_hvline_vertical_top_lsb, u8g2_cb);
}